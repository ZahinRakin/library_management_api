...
async def get_book(book_id):
    response = requests.get(f"http://localhost:8000/api/v1/books/{book_id}")
    return response.json()

async def get_total_book_count() -> int:
    response = requests.get(f"http://localhost:8000/api/v1/books")
    data = response.json()
    return len(data)

async def get_available_book_count() -> int:
    response = requests.get(f"http://localhost:8000/api/v1/books/available/count")
    data = response.json()
    return data
